            ...
            case 3:
                #try:
                    ...
                    if len(courses) > 0:
                        ...
                        NumberColumn = functions.OptionEditCourse(columns)

                        ListColumns = len(columns) + 1
                        if NumberColumn == ListColumns:
                            NewInformation = functions.DataEditCourse(columns, NumberColumn)
                            DAO.editFullCourse(Id, NewInformation, columns)
                        else:
                            #The column name that the user select is saved in "Column"
                            count = 0
                            for column in columns:
                                count = count + 1
                                if count == NumberColumn:
                                    NewInformation = functions.DataEditCourse(columns, NumberColumn)
                                    DAO.editOneCourseColumn(Id, NewInformation, column, columns)
                                    break
