...
class DataAccessObject():
   ...
   #Takes existing columns and saves them in the "Columns" list
    ...
    def editFullCourse(self, id, NewInformation, columns):
        if self.connection.is_connected():          #y se hay 2? implementar el id auto increment
            try:
                ...
                for i in range(len(columns)):
                    cursor = self.connection.cursor()
                    print(columns[i])
                    exe = ('UPDATE courses SET ' + columns[i] + ' = "'  + NewInformation[counter] + '" WHERE id = '  + str(id) + '')
                    print(exe)
                    cursor.execute(exe)
                    self.connection.commit()
                    counter = counter + 1
    ...
    def editOneCourseColumn(self, id, newinformation, column, columns):
        if self.connection.is_connected():          #y se hay 2? implementar el id auto increment
            try:
                    cursor = self.connection.cursor()
                    print(column)
                    exe = ('UPDATE courses SET ' + column + ' = "'  + newinformation + '" WHERE id = '  + str(id) + '')
                    print(exe)
                    cursor.execute(exe)
                    self.connection.commit()
